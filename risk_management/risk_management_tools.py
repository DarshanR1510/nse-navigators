...
async def t_set_stop_loss_order(symbol: str, stop_price: float, quantity: int, trader_name: str):
    """
    ...
    slm.set_stop_loss(symbol, stop_price, quantity, trader_name)
    return {"status": "success", "symbol": symbol, "stop_price": stop_price, "quantity": quantity, "trader_name": trader_name}


async def t_check_stop_loss_triggers(trader_name: str):
    """
    ...
    pm = PositionManager(trader_name=trader_name)
...
async def t_calculate_portfolio_var(trader_name: str, confidence: float = 0.95):
    """
...
async def t_get_risk_metrics(trader_name: str):
    """
    ...
    var_result = await t_calculate_portfolio_var(trader_name)
    return {"exposure": exposure, "VaR": var_result}



async def t_validate_trade_risk(symbol: str, quantity: int, entry_price: float, stop_loss: float, trader_name: str):
    """
    Tool: t_validate_trade_risk
    -----------------------------------
    Validates if a new trade (given symbol, quantity, entry, stop loss, trader_name) meets all risk management rules for the agent.
...
async def t_calculate_position_size(entry_price: float, stop_loss: float, portfolio_value: float, trader_name: str) -> int:        
    """
    ...
    Args:
        ...
        trader_name: Name of the agent for logging purposes.
    """
    pm = PositionManager(trader_name=trader_name)
    return pm.calculate_position_size(entry_price, stop_loss, portfolio_value, trader_name)


async def t_check_portfolio_exposure(trader_name: str):
    """
